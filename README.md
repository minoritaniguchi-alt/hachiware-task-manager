...
ダッシュボード業務・タスクにも個別にリンクを複数登録できます。
...
`タスク` シート：`ID / タイトル / 詳細 / 進捗メモ / ステータス / 期限 / リンク / 作成日時 / 完了日時 / 更新日時`

`ダッシュボード` シート：`ID / カテゴリ / 業務名 / 詳細 / 進捗メモ / リンク / スケジュール`

`リンク` シート：`カテゴリID / カテゴリ名 / アイテムID / 表示名 / URL / 備考`
...
2. 「API とサービス」→「有効な API とサービス」から以下の 2 つを有効化：
   - **Google Sheets API**
   - **Google Drive API**（複数デバイスでの自動スプレッドシート共有に使用）
...
プロジェクトルートに `.env.local` を作成し、取得したクライアント ID を設定します（このファイルは `.gitignore` 対象のため Git には含まれません）。

```
VITE_GIS_CLIENT_ID=<your-client-id>.apps.googleusercontent.com
```

> `.env.example` にテンプレートがあります。

デプロイは `npm run build && npm run deploy` をローカルで実行します。ビルド時に `.env.local` の値が読み込まれるため、サーバー側の設定は不要です。

### 3. アプリからログインする

アプリを起動すると Google ログイン画面が表示されます。ログインするとスプレッドシートが自動作成され、データの同期が始まります。

**複数デバイスの場合：** 「クラウドに接続」をタップすると Drive 上の既存スプレッドシートが自動検出され、PC・スマホで同じデータを共有できます。
